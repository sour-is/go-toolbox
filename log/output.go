...
func Output(logger event.Logger, calldepth int, in ...interface{}) {
	e := event.Event{Level: event.VerbDebug, Tags: make(tag.Tags), Meta: event.NewMetaInfo(calldepth + 1)}
...
func Outputs(logger event.Logger, calldepth int, level event.Level, msg string, tags ...interface{}) {
	e := event.Event{Level: level, Message: msg, Tags: tag.ReadTags(tags), Meta: event.NewMetaInfo(calldepth + 1)}
	logger.WriteEvent(&e)
}
